...
def load_trend_keywords():
    if not os.path.exists(TREND_KEYWORDS_FILE):
        # 처음 생성 시 뉴스 키워드에서 복사하거나 빈 리스트로 시작
        try:
...
def fetch_google_trend(keywords, start_date, end_date, geo='KR'):
    """
    구글 트렌드 데이터를 가져옵니다.
    """
    try:
        ...
        # 구글 트렌드는 한 번에 최대 5개 키워드까지 가능
        # 여기서는 단순화를 위해 입력된 키워드 중 상위 5개만 처리하거나 루프 필요
        # 우선 상위 5개만 처리
        kw_list = keywords[:5]
        ...
        # 결과를 Melt하여 네이버 데이터와 형식을 맞춤
        df = df.reset_index()
        df = df.melt(id_vars=['date'], var_name='키워드', value_name='트렌드지수')
        df.rename(columns={'date': '날짜'}, inplace=True)
        df['날짜'] = df['날짜'].dt.strftime('%Y-%m-%d')
        
        return df
    except Exception as e:
        print(f"구글 트렌드 API 오류: {e}")
        return pd.DataFrame()
