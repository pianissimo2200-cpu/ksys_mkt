...
RANK_KEYWORDS_FILE = "rank_keywords.json"

# 유틸리티 함수들을 collector_utils에서 가져와 사용합니다.
...
generate_signature = utils.generate_signature

# API 키를 인자로 받는 함수들을 래핑하여 기존 코드와의 호환성 유지
...
def fetch_naver_datalab_trend(keywords, start_date, end_date):
    return utils.fetch_naver_datalab_trend(keywords, start_date, end_date, CLIENT_ID, CLIENT_SECRET)


def main():
    st.set_page_config(page_title="마케팅 인사이트 대시보드", page_icon="📈", layout="wide")
    
    # 크롬 브라우저 자동번역 방지 및 달력 요일(Su, Mo, Tu...) 한글(일, 월, 화...) 강제 변환
    st.components.v1.html(
        ...
        const replaceCalendarDays = () => {
            const headers = parentDoc.querySelectorAll('[role="columnheader"]');
            headers.forEach(el => {
                const text = el.textContent.trim();
                if (dayMap[text]) {
                    el.textContent = dayMap[text];
                }
            });
        };
        ...
        setTimeout(replaceCalendarDays, 1000);
        </script>
        """,
        height=0, width=0
    )
    ...
    with st.sidebar:
        ...
        with st.form("date_setting_form"):
            date_range = st.date_input(
                ...
                max_value=today,
                help="💡 기간을 길게 설정할수록 데이터 수집에 다소 시간이 걸릴 수 있습니다."
            )
            apply_date_btn = st.form_submit_button("선택한 기간 적용", use_container_width=True)
        ...
        if selected_menu == "📰 네이버 주요 뉴스 스크랩 통합":
            ...
            render_news_manager(expanded=True) # 트렌드도 뉴스 키워드 기반으로 분석
            render_competitor_manager(expanded=False)
            render_rank_manager(expanded=False)
    ...
    # ===== TAB 3: 네이버 상위노출 현황 =====
        ...
        st.subheader(f"🏆 키워드별 '{COMPANY_NAME}' 최상단 노출 현황")
        st.markdown(
            f"네이버 블로그 검색(유사도순) 결과 상위 100위 내에 **'{COMPANY_NAME}'**가 포함된 포스팅의 최고 순위를 점검합니다.\n\n"
                ...
                if rank_results:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "최고 순위": st.column_config.TextColumn("현재 노출 순위", width="small", help=f"네이버 블로그 검색 API (유사도순) 기준 상위 100위 내에서 '{COMPANY_NAME}'가 언급된 포스팅이 위치한 가장 높은 노출 순위입니다."),
    ...
    # ===== TAB 4: 네이버 트렌드 분석 =====
        ...
        st.subheader("📊 지정 키워드별 검색 빈도 트렌드 분석")
        st.markdown(
            "선택한 키워드들의 **상대적 검색량 변화**를 시계열 차트로 분석합니다. (가장 검색량이 많았던 시점을 100으로 설정)\n\n"
            "💡 **활용 팁**: 특정 시즌에 검색량이 급증하는 키워드를 파악하여 광고 집행 시기를 결정할 수 있습니다."
        )
        
        if not keywords:
            st.info("👈 좌측 사이드바에서 분석할 키워드를 먼저 추가해 주세요.")
        else:
            col1, col2 = st.columns([0.7, 0.3])
            with col2:
                time_unit = st.selectbox("집계 단위", ["date", "week", "month"], index=2)
            
            if st.button("트렌드 데이터 수집 및 차트 생성", type="primary", use_container_width=True):
                with st.spinner("네이버 데이터랩에서 트렌드 데이터를 불러오는 중..."):
                    # 데이터랩은 최대 5개 키워드 그룹까지 동시에 비교 가능
                    # 여기서는 등록된 모든 키워드를 조회 (단, API 제한에 걸리지 않도록 주의)
                    df_trend = fetch_naver_datalab_trend(keywords, start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"))
                    
                    if not df_trend.empty:
                        # 시각화를 위해 피벗 (날짜별 키워드 트렌드)
                        chart_df = df_trend.pivot(index='날짜', columns='키워드', values='트렌드지수')
                        
                        st.success(f"최근 {len(chart_df)}개 기간의 트렌드 분석이 완료되었습니다.")
                        
                        # 차트 출력 (Streamlit 기본 차트)
                        ...
                        st.warning("해당 기간 동안의 트렌드 데이터를 불러오지 못했습니다. 키워드나 기간을 확인해 주세요.")
